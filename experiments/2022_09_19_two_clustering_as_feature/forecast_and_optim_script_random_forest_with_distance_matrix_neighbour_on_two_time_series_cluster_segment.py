...
if __name__ == "__main__":
    ...
    normalized_group_df = clusterize_demand_points(nb_cluster=4, mode="normalized")
    normal_group_df = clusterize_demand_points(nb_cluster=3)
    ...
    feature_engineering_df = feature_engineering(demand_history_df, distance_matrix_df)

    full_stacked_df = pd.concat([
        feature_engineering_df.set_index(DEMAND_POINT_INDEX_COLUMN_NAME),
        group_df
    ], axis=1).reset_index()

    forecaster_training_results = train_model_with_cross_validation(
        full_stacked_df, training_forecast_horizon
    )
    ...
    demand_history_engineered_df = feature_engineering(demand_to_predict_df, distance_matrix_df, mode="inference")

    full_stacked_demand_history_df = pd.concat([
        demand_history_engineered_df.set_index(DEMAND_POINT_INDEX_COLUMN_NAME),
        group_df
    ], axis=1).reset_index()

    forecast_df = forecast(
        full_stacked_demand_history_df, forecast_horizon, forecaster_training_results
    )
