...
from evse.optimisation._forecaster import forecaster_estimator
...
def train_model_with_cross_validation(
    ...
    **kwargs,
    ...
    exp_test_pred = pd.Series(best_pipe.predict(X_test), index=stacked_demand_history_test_df.index)
    print(
...
if __name__ == "__main__":
    ...
    a, b = (
        pd.read_csv(
            "/Users/deirdree.polak/Documents/Repos/EVSE/data/result/"
            "submission_dataframe_0.2.0_RandomForestRegressor_2022-.csv"
        ),
        pd.read_csv(
            "/Users/deirdree.polak/Documents/Repos/EVSE/data/result/" "submission_dataframe_0.2.0_Spline_2022-09-2.csv"
        ),
    )
    ...
    forecast_df = forecast(demand_to_predict_df, forecast_horizon, forecaster_training_result).clip(
        lower=0
    ...
    forecaster_training_result, predy = train_model_with_cross_validation(
        predictor_columns,
        demand_history_df,
        training_forecast_horizon,
        forecaster,
        use_spline=True,
        **{"splinet_fitted": forecaster_training_result.model_pipeline},
    )
    demand_to_predict_df = demand_history_df[["demand_point_index", "x_coordinate", "y_coordinate", "2017", "2018"]]

    forecast_df_ = forecast(demand_to_predict_df, forecast_horizon, forecaster_training_result)
    ...
    submission_dataframe.to_csv(
        result_path / f"submission_dataframe_{evse.__version__}_{forecaster['name']}_{datetime.datetime.now()}.csv",
        index=False,
    )
